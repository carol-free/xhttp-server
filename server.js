...
const HOST_HEADER = 'main-bvxea6i-q5dz6cfia7hee.fr-3.platformsh.site';  // NOUVEAU
const SNI = 'main-bvxea6i-q5dz6cfia7hee.fr-3.platformsh.site';          // NOUVEAU
const ALPN = ['h2', 'http/1.1', 'h3'];
const FP = 'chrome';

// Domaine Upsun
const DOMAIN = process.env.DOMAIN || 'main-bvxea6i-q5dz6cfia7hee.fr-3.platformsh.site';
...
const server = http.createServer((req, res) => {
    ...
    // Générer le lien VLESS (avec le nouveau domaine Upsun)
    if (url === `/${UUID}` || url === '/config' || url === `/${VPS_IP}`) {
        const vlessLink = `vless://${UUID}@${VPS_HOST}:${VPS_PORT}?type=xhttp&encryption=none&path=${encodeURIComponent(XHTTP_PATH)}&host=${HOST_HEADER}&mode=${XHTTP_MODE}&x_padding_bytes=${XHTTP_PADDING}&extra=%7B%22xPaddingBytes%22%3A%22${XHTTP_PADDING}%22%7D&security=none&fp=${FP}&alpn=${ALPN.join('%2C')}&sni=${SNI}#XHTTP-Upsun-Bridge`;
